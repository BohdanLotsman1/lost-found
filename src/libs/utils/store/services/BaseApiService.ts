...
axios.interceptors.response.use(
    ...
    errors => {
        if (errors?.response?.status === 401) localStorage.removeItem(JWT_LOCALSTORAGE_KEY);
...
export class BaseApiService {
    ...
    protected post(url: string, data?: object, config?: AxiosRequestConfig): Promise<AxiosResponse> {
    ...
    protected patch(url: string, data?: object, config?:any ): Promise<AxiosResponse> {
